...
class _MeetingScreenState extends State<MeetingView> {
  ...
  final _signButtonKey   = GlobalKey();
  final _avatarButtonKey = GlobalKey();
  OverlayEntry? _coachOverlayEntry;
  ...
  void initState() {
    ...
    if (uid != null) {
      _userSub = FirebaseFirestore.instance
        ...
        // Only kill streams when permission is explicitly revoked.
// Never force streams ON — the user controls that themselves.
        if (!micGranted && session.isMicOn) {
          await session.forceMicOff();
        }
        if (!camGranted && session.isCameraOn) {
  ...
  Future<void> _showCoachMarks() async {
    final prefs = await SharedPreferences.getInstance();
    if (prefs.getBool('coach_marks_seen') == true) return;
    if (!mounted) return;
    _showCoachMarkStep(0);
  }

  void _showCoachMarkStep(int step) {
    final keys  = [_signButtonKey, _avatarButtonKey];
    if (step >= keys.length) {
      SharedPreferences.getInstance()
          .then((p) => p.setBool('coach_marks_seen', true));
      return;
    }
    ...
    final titles    = [l10n.coachSignTitle, l10n.coachAvatarTitle];
    final descs     = [l10n.coachSignDesc,  l10n.coachAvatarDesc];
    final icons     = [Icons.sign_language, Icons.interpreter_mode];
    final isLast    = step == keys.length - 1;
    final btnLabel  = isLast ? l10n.coachGotIt : l10n.coachNext;

    _coachOverlayEntry?.remove();
    _coachOverlayEntry = OverlayEntry(
      builder: (_) => CoachMarkOverlay(
        spotlightCenter: center,
        spotlightRadius: 44,
        featureIcon: icons[step],
        title:       titles[step],
        description: descs[step],
        buttonLabel: btnLabel,
        onNext: () {
          _coachOverlayEntry?.remove();
          _coachOverlayEntry = null;
          if (isLast) {
            SharedPreferences.getInstance()
                .then((p) => p.setBool('coach_marks_seen', true));
          } else {
            _showCoachMarkStep(step + 1);
          }
        },
      ),
    );
    Overlay.of(context).insert(_coachOverlayEntry!);
  }
  ...
  // ── FIXED: renamed from _getHostPermissionFlags to _getCurrentUserPermissionFlags
  // so it correctly reads the current participant's own permission flags,
  // which are written by MeetingController.approveMicCam() into their own User doc.
  Future<Map<String, bool>> _getCurrentUserPermissionFlags() async {
    final uid = FirebaseAuth.instance.currentUser?.uid;
  ...
  Future<void> _onMicPressed() async {
    ...
    if (!_isHost) {
      // FIXED: was _getHostPermissionFlags() — now correctly reads
      // the current user's own permission flags from their User doc.
      final flags = await _getCurrentUserPermissionFlags();
  ...
  Future<void> _onFlipPressed() async {
    ...
    _signing.setFrontCamera(session.isFrontCamera);
  }
  ...
  Future<void> _onSignPressed() async {
    if (_signing.isEnabled) {
      await _signing.stopSignCapture();
      await _recognition.disable();
      _showSnackBar(
        context.l10n.signRecognitionStopped,
        Colors.grey,
        Icons.sign_language,
      );
    } else {
  ...
  Future<void> _onHandPressed() async {
    ...
    if (isRaised) {
      ...
      if (!mounted) return;
      _showSnackBar(context.l10n.handRaised, primaryOrange, Icons.pan_tool_outlined);
    }
  }
  ...
  Future<void> _onShareScreenPressed() async {
    if (_isHost) {
      if (session.isScreenSharing) {
        ...
        _showSnackBar(
            context.l10n.screenShareStopped, Colors.grey, Icons.stop_screen_share);
        return;
      }
  ...
  Future<void> _toggleAllParticipantsSharePermission() async {
    ...
    try {
      ...
      _showSnackBar(
        ...
        newValue ? primaryOrange : Colors.grey,
  ...
  Future<void> _startMyScreenShare() async {
    ...
    if (ok) {
      _showSnackBar(
          context.l10n.screenSharingStarted, primaryOrange, Icons.screen_share);
    } else {
      _showSnackBar(
