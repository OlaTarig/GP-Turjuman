...
class SignCaptioningController extends ChangeNotifier {
  ...
  // ── Latency tracking (sign→text) ───────────────────────────────────
  DateTime? _batchArrivalTime;
  final _signToTextLogger = LatencyLogger(name: 'Sign→Text', thresholdMs: 5000);
  final _inferenceLogger  = LatencyLogger(name: 'TFLite Inference', thresholdMs: 500);
  ...
  void setFrontCamera(bool isFront) => _isFrontCamera = isFront;
  ...
  Future<void> startSignCapture() async {
    if (!_initialized) await initialize();
    if (isEnabled) return;
  ...
  Future<void> stopSignCapture() async {
    isEnabled          = false;
  ...
  Future<void> startCapture() async {
    if (!isEnabled) await startSignCapture();
  }
  ...
  void _onKeypointEvent(dynamic event) {
    if (event is String) {
      ...
      _batchArrivalTime = DateTime.now();
      ...
      // Native always mirrors for front camera. When using the back camera,
      // negate all X values (every 3rd value) to undo the incorrect mirror.
      if (!_isFrontCamera) {
        for (int i = 0; i < flat.length; i += 3) flat[i] = -flat[i];
      }
  ...
  Future<void> _runInferenceFromFlat(List<double> flat) async {
    ...
    try {
      ...
      _inferenceLogger.record(inferenceMs);
  ...
  void _flushSentence() {
    ...
    if (_batchArrivalTime != null) {
      final e2eMs = DateTime.now().difference(_batchArrivalTime!).inMilliseconds;
      _signToTextLogger.record(e2eMs);
      _batchArrivalTime = null;
    }
  ...
  // ── Test entry points ─────────────────────────────────────────────
  Future<void> runInferenceForTest(List<double> flat) =>
      _runInferenceFromFlat(flat);

  void onSignRecognizedForTest(String word) => _onSignRecognized(word);
