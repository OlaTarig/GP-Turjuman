...
class ZegoSessionController extends ChangeNotifier {
  static final int    appID   = int.tryParse(dotenv.env['ZEGO_APP_ID'] ?? '') ?? 0;
  ...
  Future<void> startPublishing() async {
    ...
    // startPreview must be called before startPublishingStream for video
    // to be included in the stream. Call without canvas first to initialize
    // the capture pipeline; the canvas will be attached once the view renders.
  ...
  Future<bool> toggleCameraWithPermission() async {
    ...
    if (!isCameraOn) {
      ...
      if (viewId != null) {
        ...
        // View not ready yet — startPreview will be called from the
        // createCanvasView callback in startPublishing once the view renders.
        ZegoExpressEngine.instance.startPreview();
      }
    }
